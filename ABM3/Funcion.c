...
void altaAlmuerzo(eAlmuerzo alm[], int tamAlm, eComida com[], int tamCom, eEmpleado emp[], int tamEmp, eSector sec[], int tamSec, int *pId)
{
    ...
    else
    {
        ...
        mostrarEmpleados(emp, tamEmp, sec, tamSec);
...
void mostrarAlmuezo(eEmpleado unEmpleado, eSector sectores[], int tam, eComida com[], eAlmuerzo alm, eFecha fecha)
{
    char descSector[20];
    char descComida[20];
    ...
    printf("%4d   %2d/%2d/%2d   %c     %4d     %s     %c\n", alm.id, fecha.dia, fecha.mes, fecha.anio, descComida, unEmpleado.legajo, unEmpleado.nombre, descSector);
}


void mostrarAlmuerzos(eEmpleado lista[], int tam, eSector sectores[], int tamSector)
{

    system("cls");
    printf("ID   Fecha   Comida   Legajo   Nombre  Sector\n\n");
...
void obtenerComida(eSector sectores[], int tam, int idSector, char cadena[])
{
    for(int i=0; i < tam; i++)
        {
            if( sectores[i].id == idSector)
